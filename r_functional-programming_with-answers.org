...
*** Important note on notation: formulas
...
#+BEGIN_SRC R
banding %>% map_if(is.character, as.factor)
#+END_SRC

However, src_R[:eval no]{map_if} and src_R[:eval no]{map_at} always return lists. If you want the output to be of the same type of the input (a data frame in our case), use src_R[:eval no]{modify_if} and src_R[:eval no]{modify_at} instead.

#+BEGIN_SRC R
banding %>% modify_if(is.character, as.factor)
#+END_SRC

#+BEGIN_RED
Note that this could also be accomplished with src_R[:eval no]{mutate_if()}:

#+BEGIN_SRC R
banding %>% mutate_if(is.character, as.factor)
#+END_SRC
...
** Apply several functions to elements of a list

#+BEGIN_SRC R
invoque_map(c("mean", "var", "sum", "max"), banding[4:6])
#+END_SRC
...
#+BEGIN_VERSE
- Do we want to apply several functions?
#+END_VERSE
...
- The [[http://r4ds.had.co.nz/iteration.html#shortcuts][iteration chapter]] of [[http://hadley.nz/][Hadley Wickham]]'s book [[http://r4ds.had.co.nz/index.html][R for data science]]
